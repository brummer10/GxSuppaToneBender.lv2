...
#define GTK_TBENDER_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_TBENDER_KNOB, GtkTBenderKnobPrivate))

G_DEFINE_TYPE(GtkTBenderKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkTBenderKnobPrivate GtkTBenderKnobPrivate;

struct _GtkTBenderKnobPrivate
{
	int knob_x;
	int knob_y;
	int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
	/** check resize **/
	int grow;
 	GtkTBenderKnobPrivate *priv = GTK_TBENDER_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model) {
        ...
        cairo_move_to (cr, knobx1-knob_x/2.2-extents.width/2, knoby1+knob_y/1.9+extents.height/2);
        cairo_show_text(cr, "Off");

        cairo_text_extents(cr,"On ", &extents);
        cairo_move_to (cr, knobx1+knob_x/2.6-extents.width/2, knoby1+knob_y/1.9+extents.height/2);
...
{
	g_assert(GTK_IS_TBENDER_KNOB(widget));
	GtkTBenderKnob *knob = GTK_TBENDER_KNOB(widget);
	GtkTBenderKnobPrivate *priv = GTK_TBENDER_KNOB_GET_PRIVATE(knob);
...
/****************************************************************
 ** init the GtkTBenderKnobClass
 */

static void gtk_knob_class_init (GtkTBenderKnobClass *klass)
{
...
//--------- connect the events with funktions
	...
	g_type_class_add_private(obj_class, sizeof (GtkTBenderKnobPrivate));
}
...
static void gtk_knob_init (GtkTBenderKnob *knob)
{
...
{
	GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_TBENDER_KNOB, NULL ));
