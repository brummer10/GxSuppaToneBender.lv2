...
static void resize_event(gx_vstbUI *ui);
...
cairo_surface_t *cairo_image_surface_create_from_stream (gx_vstbUI* ui, const unsigned char* name) {
...
static LV2UI_Handle instantiate(const LV2UI_Descriptor * descriptor,
	...
	gx_vstbUI* ui = (gx_vstbUI*)malloc(sizeof(gx_vstbUI));
	...
	//ui->controls[0] = (gx_controller) {{ 1.0, 1.0, 0.0, 1.0, 1.0},{ 50, 60, 61, 61}, false,"POWER", BSWITCH, BYPASS};
	ui->controls[0] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 40, 30, 61, 61}, false,"FUZZ", KNOB, FUZZ};
	ui->controls[1] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 120, 30, 61, 61}, false,"TONE", KNOB, TONE};
	ui->controls[2] = (gx_controller) {{ 0.5, 0.5, 0.0, 1.0, 0.01},{ 200, 30, 61, 61}, false,"VOLUME", KNOB, VOLUME};
	ui->block_event = -1;
...
static void knob_expose(gx_vstbUI *ui,gx_controller* knob) {
...
static void bypass_expose(gx_vstbUI *ui, gx_controller* switch_) {
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_CLEAR);
	cairo_paint(ui->crf);
	cairo_set_operator(ui->crf,CAIRO_OPERATOR_OVER);
	if (switch_->is_active == true) {
		cairo_set_source_rgba (ui->crf, 0.8, 0.8, 0.8,0.8);
		cairo_rectangle (ui->crf,6,6,49,48);
		cairo_fill(ui->crf);
		cairo_stroke(ui->crf);
	}
...
static void draw_controller(gx_vstbUI *ui, gx_controller* controller) {
...
static void _expose(gx_vstbUI *ui) {
	static const char* plug_name = "GxSuppaToneBender" ;
...
static void controller_expose(gx_vstbUI *ui, gx_controller * control) {
	cairo_push_group (ui->cr);
	cairo_scale (ui->cr, ui->rescale.x, ui->rescale.y);
...
static void send_controller_event(gx_vstbUI *ui, int controller) {
...
static void check_value_changed(gx_vstbUI *ui, int i, float* value) {
...
static void check_is_active(gx_vstbUI *ui, int i, bool set) {
...
bool aligned(int x, int y, gx_controller *control, gx_vstbUI *ui) {
...
static void scroll_event(gx_vstbUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
...
static void set_key_value(gx_vstbUI *ui, int set_value) {
	float value = 0.0;
	for (int i=0;i<CONTROLS;i++) {
...
static void key_event(gx_vstbUI *ui, int direction) {
	float value;
	for (int i=0;i<CONTROLS;i++) {
		if (ui->controls[i].is_active) {
			value = min(ui->controls[i].adj.max_value,max(ui->controls[i].adj.min_value, 
...
static void set_previous_controller_active(gx_vstbUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
...
static void set_next_controller_active(gx_vstbUI *ui) {
	for (int i=0;i<CONTROLS;i++) {
		if(ui->controls[i].is_active == true) {
			ui->controls[i].is_active = false;
			send_controller_event(ui, i);
...
void get_active_controller(gx_vstbUI *ui, bool set) {
...
static void motion_event(gx_vstbUI *ui, double start_value, XMotionEvent *mo) {
...
static void event_handler(gx_vstbUI *ui) {
	XEvent xev;
